...
def correct_RS_effect(img, homo_mixtures):
    return transformImage(img, homo_mixtures.reshape(-1, 3, 3), homo_mixtures.shape[0])


def RS_rect_warp_img_make_gif(frames_path_ois_off, match_frames_path_ois_off, gyro_homs_intra, gyro_homs_inter, gif_path=None, thres=None):
    img_cc9pro = (cv2.imread(i) for i in frames_path_ois_off)
    match_img_cc9pro = (cv2.imread(i) for i in match_frames_path_ois_off)
    ...
    for i, _ in enumerate(gyro_homs_inter):

        if i == len(gyro_homs_inter) - 1:
            break
        ...
        img_orig = correct_RS_effect(img_orig, gyro_homs_intra[i])
        img_match = correct_RS_effect(img_match, gyro_homs_intra[i + 1])

        height, width = img_orig.shape[:2]
        warp_gyro = cv2.warpPerspective(img_orig, gyro_homs_inter[i][0].reshape(3, 3), (width, height))
